...
from django.forms.widgets import RadioFieldRenderer, RadioChoiceInput,Input
...
class AdminCheckbox(forms.CheckboxInput):
    def __init__(self, attrs=None):
        final_attrs = {'class': 'checkboxinput none'}
        if attrs is not None:
            final_attrs.update(attrs)
        super(AdminCheckbox, self).__init__(attrs=final_attrs)
    ...
    def render(self, name, value, attrs=None):
        ...
        return mark_safe(u'%s <p><i></i></p>' % html)
